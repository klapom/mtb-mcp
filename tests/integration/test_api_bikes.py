...
@pytest.mark.asyncio
async def test_list_bikes_empty(authed_client: AsyncClient) -> None:
    """GET /api/v1/bikes/ returns empty list when no bikes exist."""
    resp = await authed_client.get("/api/v1/bikes/")
...
async def test_add_component_creates_bike_and_component(
    ...
    resp2 = await authed_client.get("/api/v1/bikes/")
    body2 = resp2.json()
...
@pytest.mark.asyncio
async def test_log_ride(authed_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_log_service(authed_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_service_resets_wear(authed_client: AsyncClient) -> None:
    ...
    resp_before = await authed_client.get("/api/v1/bikes/WearBike/maintenance")
    ...
    resp_after = await authed_client.get("/api/v1/bikes/WearBike/maintenance")
